...
Deno.serve(async req=>{
  ...
  let body:{full_name?:string;email?:string;role?:typeof allowedRoles[number];lot_ids?:string[]};
  try{body=await req.json()}catch{return reply({error:"JSON inválido"},400)}
  const fullName=body.full_name?.trim(),email=body.email?.trim().toLowerCase(),lotIds=body.lot_ids??[],role=body.role;
  ...
  const {data:requester}=await admin.from("parking_memberships").select("role,active,lot_ids").eq("business_id",businessId).eq("user_id",auth.user.id).maybeSingle();
  ...
  const {data:membership,error:membershipError}=await admin.from("parking_memberships").insert({business_id:businessId,user_id:created.user.id,full_name:fullName,role,role_id:roleRow?.id??null,lot_ids:lotIds,active:true}).select("id").single();
  ...
  return reply({user:{id:membership.id,user_id:created.user.id,full_name:fullName,email,role,lot_ids:lotIds,temporary_password:password}});
});
