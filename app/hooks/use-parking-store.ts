...
export type PlatformBusiness={id:string;name:string;slug:string;active:boolean;createdAt:string;ownerName:string;planType:"demo"|"monthly"|"annual";planExpiresAt:string|null;maxLots:number};
...
export function useParkingStore(){
  ...
  const [platformBusinesses,setPlatformBusinesses]=useState<PlatformBusiness[]>([]),[platformLots,setPlatformLots]=useState<ParkingLot[]>([]),[selectedBusinessId,setSelectedBusinessId]=useState("");
  ...
  async function loadRemote(businessOverride?:string){
    ...
    try{
      ...
      if(platformAdmin&&!platformBusinessId){const [{data:businessRows,error:businessError},{data:allLots,error:lotError},{data:allMembers}]=await Promise.all([supabase.from("parking_businesses").select("id,name,slug,active,created_at,plan_type,plan_expires_at,max_lots").order("name"),supabase.from("parking_lots").select("id,business_id,name,code,capacity,active").eq("active",true).order("name"),supabase.from("parking_memberships").select("id,business_id,user_id,full_name,role,lot_ids,permission_codes,active").order("full_name")]);if(businessError||lotError)throw businessError??lotError;const mapped:ParkingLot[]=(allLots??[]).map(r=>({id:String(r.id),businessId:String(r.business_id),name:String(r.name),code:String(r.code),capacity:Number(r.capacity),active:Boolean(r.active)}));const owners=new Map((allMembers??[]).filter((m:any)=>m.role==="owner"&&m.active).map((m:any)=>[String(m.business_id),String(m.full_name)]));setPlatformBusinesses((businessRows??[]).map((r:any)=>({id:String(r.id),name:String(r.name),slug:String(r.slug),active:Boolean(r.active),createdAt:String(r.created_at),ownerName:owners.get(String(r.id))??"Sin propietario asignado",planType:["monthly","annual"].includes(String(r.plan_type))?r.plan_type:"demo",planExpiresAt:r.plan_expires_at?String(r.plan_expires_at):null,maxLots:Number(r.max_lots??1)})));setPlatformLots(mapped);setLots(mapped);setLotId("");setBusinessName("Administración de plataforma");setProfile({id:user.id,fullName:String(user.user_metadata?.full_name||user.email||"Super administrador"),role:"super_admin",allowedLotIds:mapped.map(l=>l.id),permissionCodes:["*"]});remote.current=null;setStays([]);setPayments([]);setStaff((allMembers??[]).map((m:any)=>({id:String(m.id),fullName:String(m.full_name),email:"Usuario registrado",role:String(m.role),lotIds:(m.lot_ids??[]).map(String),permissionCodes:(m.permission_codes??[]).map(String),status:m.active?"active":"inactive"})));setSource("supabase");setAuthState("authenticated");setSyncError("");return;}
      ...
      setLots(mappedLots);setLotId(selected);setBusinessName(String(businessRow?.name||user.user_metadata?.business_name||"Empresa"));setProfile({id:user.id,fullName:effectiveName,role:effectiveRole,allowedLotIds:allowed,permissionCodes:platformAdmin?["*"]:permissionCodes});setStays(mappedStays);setPayments(mappedPayments);setVehicleTypes(mappedVehicleTypes);setRates(mappedRates);setCashRegisters((registerRows??[]).map((r:any)=>({id:String(r.id),businessId:String(r.business_id),lotId:String(r.lot_id),name:String(r.name),code:String(r.code),active:Boolean(r.active)})));setCashCuts((cutRows??[]).map((r:any)=>({id:String(r.id),lotId:String(r.lot_id),cashRegisterId:r.cash_register_id?String(r.cash_register_id):undefined,openedAt:String(r.opened_at),closedAt:String(r.closed_at),openedBy:userNames.get(String(r.opened_by))??"Usuario",closedBy:userNames.get(String(r.closed_by))??"Usuario",openingCash:Number(r.opening_cash),expectedCash:Number(r.expected_cash??0),countedCash:Number(r.counted_cash??0),notes:r.notes?String(r.notes):undefined})));
  ...
  async function leavePlatformBusiness(){if(profile.role!=="super_admin")return;setSelectedBusinessId("");remote.current=null;setSource("loading");await loadRemote("");}
  async function signOut(){if(supabase)await supabase.auth.signOut();remote.current=null;setSelectedBusinessId("");setPlatformBusinesses([]);setPlatformLots([]);setLots(fallbackLots);setLotId("centro");setBusinessName("Empresa");setProfile(fallbackProfile);setStays(fallbackStays);setPayments([]);setRate(fallbackRates[0]);setRates(fallbackRates);setVehicleTypes(fallbackVehicleTypes);setShift(fallbackShift);setCashRegisters(fallbackRegisters);setCashCuts([]);setStaff([]);setSource("fallback");setAuthState("unauthenticated");setSyncError("Sesión cerrada");}
