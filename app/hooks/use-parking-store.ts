...
import type { CashCut, CashRegister, ParkingLot, ParkingStay, Payment, Profile, RatePlan, Shift } from "@/app/types/parking";
...
export function useParkingStore(){
  ...
  const [stays,setStays]=useState(fallbackStays),[payments,setPayments]=useState<Payment[]>([]),[rate,setRate]=useState(fallbackRate),[shift,setShift]=useState(fallbackShift),[cashRegisters,setCashRegisters]=useState<CashRegister[]>(fallbackRegisters),[cashCuts,setCashCuts]=useState<CashCut[]>([]);
  ...
  async function loadRemote(){
    ...
    try{
      ...
      const [{data:lotRows,error:lotError},{data:rateRows},{data:stayRows,error:stayError},{data:paymentRows},{data:shiftRows},{data:businessRow},{data:registerRows},{data:cutRows},{data:directoryRows}]=await Promise.all([
        ...
        supabase.from("parking_stays").select("id,folio,lot_id,entered_at,exited_at,status,qr_token,barcode_value,amount_due,parking_vehicles(plate,make,model,color)").eq("business_id",businessId).in("status",["active","pending_payment","paid"]).order("entered_at",{ascending:false}).limit(500),
      ...
      setLots(mappedLots);setLotId(selected);setBusinessName(String(businessRow?.name||user.user_metadata?.business_name||"Empresa"));setProfile({id:user.id,fullName:String(membership.full_name||user.email||"Usuario"),role:membership.role,allowedLotIds:allowed});setStays(mappedStays);setPayments(mappedPayments);setCashRegisters((registerRows??[]).map((r:any)=>({id:String(r.id),businessId:String(r.business_id),lotId:String(r.lot_id),name:String(r.name),code:String(r.code),active:Boolean(r.active)})));setCashCuts((cutRows??[]).map((r:any)=>({id:String(r.id),lotId:String(r.lot_id),cashRegisterId:r.cash_register_id?String(r.cash_register_id):undefined,openedAt:String(r.opened_at),closedAt:String(r.closed_at),openedBy:userNames.get(String(r.opened_by))??"Usuario",closedBy:userNames.get(String(r.closed_by))??"Usuario",openingCash:Number(r.opening_cash),expectedCash:Number(r.expected_cash??0),countedCash:Number(r.counted_cash??0),notes:r.notes?String(r.notes):undefined})));
  ...
  return{profile,businessName,lots,lotId,setLotId,lot,stays,active,payments,rate:currentRate,setRate,saveRate,shift,setShift,cashRegisters,cashCuts,openShift,closeShift,createCashRegister,registerEntry,charge,calculate:(stay:ParkingStay)=>calculateFee(stay.enteredAt,currentRate),source,syncError,reload:loadRemote,authState,signIn,signUp,registerBusiness,signOut,continueDemo,staff,inviteStaff,updateStaff,createBusiness,canManageStaff};
}
