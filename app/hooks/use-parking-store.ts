...
export function useParkingStore(){
  ...
  async function loadRemote(){
    if(!supabase){setSource("fallback");return;}
    try{
      const {data:{user}}=await supabase.auth.getUser();
      if(!user){setSource("fallback");setSyncError("Inicia sesión para sincronizar con Supabase");return;}
      const {data:membership,error:membershipError}=await supabase.from("parking_memberships").select("business_id,full_name,role,lot_ids").eq("user_id",user.id).eq("active",true).maybeSingle();
      ...
      setSource("supabase");setSyncError("");
    }catch(error){console.warn("ParkFlow fallback:",error);remote.current=null;setSource("fallback");setSyncError(error instanceof Error?error.message:"No fue posible consultar Supabase");}
  }
  ...
  async function signIn(email:string,password:string){if(!supabase)throw new Error("Faltan las credenciales públicas de Supabase");const {error}=await supabase.auth.signInWithPassword({email,password});if(error)throw error;setSource("loading");await loadRemote();}
  async function signOut(){if(supabase)await supabase.auth.signOut();remote.current=null;setLots(fallbackLots);setLotId("centro");setProfile(fallbackProfile);setStays(fallbackStays);setPayments([]);setRate(fallbackRate);setShift(fallbackShift);setSource("fallback");setSyncError("Sesión cerrada");}
  return{profile,lots,lotId,setLotId,lot,stays,active,payments,rate:currentRate,setRate,saveRate,shift,setShift,registerEntry,charge,calculate:(stay:ParkingStay)=>calculateFee(stay.enteredAt,currentRate),source,syncError,reload:loadRemote,signIn,signOut};
}
