...
export function useParkingStore(){
  ...
  async function loadRemote(){
    ...
    try{
      ...
      if(platformAdmin){const [{data:allLots,error},{data:allMembers}]=await Promise.all([supabase.from("parking_lots").select("id,business_id,name,code,capacity,active").eq("active",true).order("name"),supabase.from("parking_memberships").select("id,user_id,full_name,role,lot_ids,permission_codes,active").order("full_name")]);if(error)throw error;const mapped:ParkingLot[]=(allLots??[]).map(r=>({id:String(r.id),businessId:String(r.business_id),name:String(r.name),code:String(r.code),capacity:Number(r.capacity),active:Boolean(r.active)}));setLots(mapped);setLotId(mapped[0]?.id??"");setBusinessName("Administración de plataforma");setProfile({id:user.id,fullName:String(user.user_metadata?.full_name||user.email||"Super administrador"),role:"super_admin",allowedLotIds:mapped.map(l=>l.id),permissionCodes:["*"]});remote.current={businessId:mapped[0]?.businessId??"",userId:user.id};setStays([]);setPayments([]);setStaff((allMembers??[]).map((m:any)=>({id:String(m.id),fullName:String(m.full_name),email:"Usuario registrado",role:String(m.role),lotIds:(m.lot_ids??[]).map(String),permissionCodes:(m.permission_codes??[]).map(String),status:m.active?"active":"inactive"})));setSource("supabase");setAuthState("authenticated");setSyncError("");return;}
      if(membershipError||!membership)throw membershipError??new Error("Sin membresía activa");
      const businessId=String(membership.business_id); remote.current={businessId,userId:user.id};const {data:accessContext}=await supabase.rpc("get_my_parking_context").maybeSingle();const permissionCodes=((accessContext as any)?.permission_codes??[]) as string[];
      ...
      setLots(mappedLots);setLotId(selected);setBusinessName(String(businessRow?.name||user.user_metadata?.business_name||"Empresa"));setProfile({id:user.id,fullName:String(membership.full_name||user.email||"Usuario"),role:membership.role,allowedLotIds:allowed,permissionCodes});setStays(mappedStays);setPayments(mappedPayments);setVehicleTypes(mappedVehicleTypes);setRates(mappedRates);setCashRegisters((registerRows??[]).map((r:any)=>({id:String(r.id),businessId:String(r.business_id),lotId:String(r.lot_id),name:String(r.name),code:String(r.code),active:Boolean(r.active)})));setCashCuts((cutRows??[]).map((r:any)=>({id:String(r.id),lotId:String(r.lot_id),cashRegisterId:r.cash_register_id?String(r.cash_register_id):undefined,openedAt:String(r.opened_at),closedAt:String(r.closed_at),openedBy:userNames.get(String(r.opened_by))??"Usuario",closedBy:userNames.get(String(r.closed_by))??"Usuario",openingCash:Number(r.opening_cash),expectedCash:Number(r.expected_cash??0),countedCash:Number(r.counted_cash??0),notes:r.notes?String(r.notes):undefined})));
